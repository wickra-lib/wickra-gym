...
takes the header and library from `WKGYM_INC` / `WKGYM_LIB`, supplied from
outside, and compiles the hand-written `src/wickra_gym.c` against whatever
...
This makes the pairing explicit:
  ...
  * Every `wickra_*` symbol `src/wickra_gym.c` calls must be declared in
    `bindings/c/include/wickra_gym.h`. A call to a symbol that is not
...
downloads a prebuilt `wickra-gym-c-<triple>.tar.gz` for the version in
...
HEADER = "bindings/c/include/wickra_gym.h"
